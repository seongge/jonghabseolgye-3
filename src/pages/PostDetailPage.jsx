...
export default function PostDetailPage({ setPage, post, setPosts, posts, setSelectedPost, isLoggedIn, showToast, addNotification }) {
  ...
  const handlePostLike = async () => {
    ...
    try {
      ...
      // 서버 API 호출 (post_id는 post.id에 저장되어 있음)
      ...
      // 서버 응답 메시지에 따라 상태 업데이트
      const isAdded = response.data.message.includes('추가');
      ...
      const updatedPosts = posts.map(p => {
        if (p.id === post.id) {
          return {
            ...p,
            likes: isAdded ? p.likes + 1 : Math.max(0, p.likes - 1),
            isLiked: isAdded
          };
        }
        return p;
      ...
      // 현재 상세 페이지의 게시글 상태 업데이트
      setSelectedPost({
        ...post,
        likes: isAdded ? post.likes + 1 : Math.max(0, post.likes - 1),
        isLiked: isAdded
      });
