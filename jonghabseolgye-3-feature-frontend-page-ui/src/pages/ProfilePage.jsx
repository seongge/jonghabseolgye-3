...
export default function ProfilePage({ isLoggedIn, setIsLoggedIn, setPage, posts, setBoardCategory, showToast }) {
  const myPostsCount = posts.filter(p => p.author === '익명(나)').length;
  const likedPostsCount = posts.filter(p => p.isLiked).length;
  const myCommentsCount = posts.filter(p => p.comments?.some(c => c.author === '익명(나)')).length;
        ...
        {!isLoggedIn ? (
          ...
          <>
            <div style={profileCardStyle}>
              ...
              <h3 style={{ fontSize: '20px', fontWeight: 'bold', margin: '15px 0 5px' }}>홍길동 (2021123456)</h3>
              <p style={{ color: '#868e96', fontSize: '14px' }}>컴퓨터공학과</p>
            </div>

            <div style={{ ...menuItemStyle, marginBottom: '12px' }} onClick={() => { setBoardCategory('내가 작성한 글'); setPage('board'); }}>
            ...
            <div style={{ ...menuItemStyle, marginBottom: '12px' }} onClick={() => { setBoardCategory('내가 작성한 댓글'); setPage('board'); }}>
            ...
            <div style={menuItemStyle} onClick={() => { setBoardCategory('좋아요 표시한 글'); setPage('board'); }}>
              <div style={menuLabelStyle}><IoMdHeart size={20} color="#FF3366" /><span>좋아요 표시한 글</span></div>
              <span style={{ ...countStyle, color: '#FF3366' }}>{likedPostsCount}개</span>
            </div>

            <button style={logoutButtonStyle} onClick={() => { setIsLoggedIn(false); setPage('main'); if(showToast) showToast('로그아웃 되었습니다.'); }}>
