...
export default function MainPage({ setPage, unreadCount, setBoardCategory, posts, isLoggedIn, showToast }) {
    const schoolNoticePreview = posts.filter(p => p.category === '학교 공지').slice(0, 4);
    const deptNoticePreview = posts.filter(p => p.category === '학과 공지').slice(0, 4);
