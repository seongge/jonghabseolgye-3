...
export default function NotificationPage({ setPage, notifications, setNotifications, posts, setSelectedPost, showToast }) {

  const handleItemClick = async (notif) => {
    try {
      if (notif.id) {
        await api.delete(`/api/alarms/${notif.id}`);
      }
    ...
    const targetId = notif.targetPostId || notif.post_id || notif.targetNoticeId || notif.notice_id;
    const targetPost = posts.find(p => p.id === targetId || p._id === targetId || String(p.id) === String(targetId) || String(p._id) === String(targetId));
    if (targetPost) {
      setSelectedPost(targetPost);
      setPage('postDetail');
    } else if (showToast) {
      showToast('해당 게시글 또는 공지를 찾을 수 없습니다.');
    } else {
      alert('해당 게시글 또는 공지를 찾을 수 없습니다.');
    }
  };
