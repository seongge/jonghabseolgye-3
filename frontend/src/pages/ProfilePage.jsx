...
export default function ProfilePage({ isLoggedIn, setIsLoggedIn, setPage, posts, setBoardCategory, showToast }) {
  ...
  const countMyCommentsInPost = (post) => {
    if (Number(post.myCommentCount) > 0) return Number(post.myCommentCount);
    if (Number(post.my_comment_count) > 0) return Number(post.my_comment_count);
    if (!Array.isArray(post.comments)) return 0;
    return post.comments.filter(isMyComment).length;
  };

  // 내가 작성한 글 카운트
  const myPostsCount = posts ? posts.filter(isMyPost).length : 0;

  // 좋아요 표시한 글 카운트
  const likedPostsCount = posts 
    ? posts.filter(p => p.isLiked === true || p.is_liked === true || (Array.isArray(p.likedUsers) && p.likedUsers.includes(currentUserEmail))).length 
    : 0;

  // 내가 작성한 댓글 카운트
  const myCommentsCount = posts 
    ? posts.reduce((total, post) => total + countMyCommentsInPost(post), 0)
    : 0;
